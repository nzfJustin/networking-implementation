...
public:
  ...
  SimpleRouter();
  void prepareForReplyARP( arp_hdr* arp_hdr, const Interface* iface);
  void ResponseARP(arp_hdr* packet_arp_hdr, const Interface* iface);
  void packetCheck(ip_hdr* packet_ip_hdr);
  void forwardThePacket(ip_hdr* packet_ip_hdr, uint8_t* packet_ether_hr, const Interface* iface, uint8_t* whole_packet, const Buffer& packet);
